...
import { ArrowRight, FileSignature, CalendarPlus } from "lucide-react";
...
export default function AuditThankYouPage() {
  ...
  const formLink = currency === "CAD" 
    ? "https://forms.gle/N1i6c9VcnCtP2PjA7" 
    : "https://forms.gle/zLjqY2yMNfAXEbeX6";
  
  const calendarLink = "https://calendar.app.google/nET7dBGm5i2cxE2x9";
        ...
        <div className="max-w-2xl mx-auto space-y-8">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5 }}>
            ...
            <p className="text-lg text-brand-warm-gray mt-4">
              Thank you for signing up for the <span className="font-bold text-brand-charcoal">Audit & Action Session</span>. There are just two quick steps left to officially kick things off.
            </p>
          </motion.div>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 0.5 }} className="bg-brand-white border-2 border-brand-border rounded-3xl p-8 shadow-xl text-left space-y-6">
            <div className="space-y-3">
              ...
              <p className="text-sm text-brand-warm-gray">
                Please review and sign the coaching agreement so we are both on the same page before our session.
              </p>
              <a 
